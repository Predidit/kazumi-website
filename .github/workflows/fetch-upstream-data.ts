...
async function fetchAllContributors(): Promise<Contributor[]> {
  ...
  while (hasNextPage) {
    try {
      ...
      const linkResponse = await fetch(`${CONTRIBUTORS_API}?per_page=${PER_PAGE}&page=${page}`);
...
function isDataEqual(newData: unknown, oldData: unknown): boolean {
  ...
  const newCopy = JSON.parse(JSON.stringify(newData)) as Record<string, unknown>;
  const oldCopy = JSON.parse(JSON.stringify(oldData)) as Record<string, unknown>;
...
async function main(): Promise<void> {
  try {
    ...
    const oldContributorsData = readJSONSafe<ContributorsData>(contributorsPath);
    ...
    if (contributorsChanged) {
      writeFileSync(contributorsPath, `${JSON.stringify(newContributorsData, null, 2)}\n`, "utf8");
    ...
    if (releasesChanged) {
      writeFileSync(releasesPath, `${JSON.stringify(newReleasesData, null, 2)}\n`, "utf8");
